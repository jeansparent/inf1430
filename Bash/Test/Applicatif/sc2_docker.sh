...
for i in {1..5}; do
    ...
    echo "========== PIDSTAT REPORT for gunicorn =========="
    cpu_values=$(grep -a "gunicorn" pidstat_cpu.log | tr -s ' ' | cut -d ' ' -f 8)
    ...
    mem_values=$(grep -a "gunicorn" pidstat_mem.log | tr -s ' ' | cut -d ' ' -f 7)
    ...
    echo "Max CPU usage for gunicorn: $cpu_max %"
    echo "Mean CPU usage for gunicorn: $cpu_mean %"
    echo "Max MEM (RSS) for gunicorn: $mem_max KB"
    echo "Mean MEM (RSS) for gunicorn: $mem_mean KB"
    echo "============================================="
done